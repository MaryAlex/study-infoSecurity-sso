...
export class Objects extends React.Component<IObjectsProps, ObjectsState> {
    ...
    private getTable = (): React.ReactNode => {
        const table = this.state.objects.map(this.getRow);
        const header = this.state.objects.length && this.getHeader();
        if (this.state.isCreateMode) {
            table.push(<EditRow cancel={this.cancelCreateNew}
                                object={newObjectTypes(this.props.object)}
                                updateObject={this.addObject}/>);
        }  else {
            table.push(<Button bsStyle="primary" onClick={this.openCreateNew}>Create</Button>)
        }
        return (
            <Grid>
                {header}
    ...
    private getRow = (object: any): React.ReactNode => {
        const row = Object.keys(object).reduce<React.ReactNode[]>(getForEachHandlerNodeType(this.getCol, object), []);
        row.push(this.getCol((object[TYPE_KEY] as Type).name));
        row.push(this.getEditButton(object));
        row.push(this.getDeleteButton(object));
    ...
    private getCol = (text: string): React.ReactNode => {
        return (
            <Col className="my-row" key={text} xs={12} md={1}>{text}</Col>
        )
    }
